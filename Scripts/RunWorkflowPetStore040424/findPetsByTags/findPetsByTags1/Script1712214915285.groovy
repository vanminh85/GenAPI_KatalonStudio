...
def petPayload = '{"name": "pet__unique__", "photoUrls": ["url1", "url2"], "category": {"id": 1, "name": "category__unique__"}, "tags": [{"id": 1, "name": "tag1__unique__"}], "status": "available"}'
petRequest.setBodyContent(new HttpTextBodyContent(replaceSuffixWithUUID(petPayload)))
def petResponse = WSBuiltInKeywords.sendRequest(petRequest)
WSBuiltInKeywords.verifyResponseStatusCode(petResponse, 200)

// Step 3: Execute POST /pet/findByTags with valid tags
def tags = "tag1,tag2"
def findByTagsRequest = new RequestObject()
findByTagsRequest.setRestUrl("https://petstore.swagger.io/v2/pet/findByTags?tags=${tags}")
