...
// Step 9: Update the Scheduler
def updatedSchedulerRequest = new RequestObject()
updatedSchedulerRequest.setRestUrl("https://testops.katalon.io/api/v1/test-projects/${projectId}/schedulers/${schedulerId}")
updatedSchedulerRequest.setRestRequestMethod("PUT")
addAuthHeader(updatedSchedulerRequest)
addContentTypeHeader(updatedSchedulerRequest)
def updatedSchedulerPayload = new HttpTextBodyContent(replaceSuffixWithUUID(JsonOutput.toJson([
    "name": "UpdatedTestScheduler__${uuid}__",
    "startTime": "2022-01-01T01:00:00Z",
    "interval": 30,
    "intervalUnit": "MINUTE",
    "runConfigurationId": runConfigId
])))
def updatedSchedulerResponse = WSBuiltInKeywords.sendRequest(updatedSchedulerRequest)

// Step 10: Verify the response status code is 200
def statusCode = updatedSchedulerResponse.getStatusCode()
if (statusCode == 200) {
    println("Test case passed: Status code is 200")
} else {
    println("Test case failed: Status code is not 200")
}
