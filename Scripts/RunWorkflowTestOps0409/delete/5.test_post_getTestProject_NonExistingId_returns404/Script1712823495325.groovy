...
// Step 5: Get the TestProject with a non-existing id
def nonExistingId = 9999
def nonExistingRequest = new RequestObject()
nonExistingRequest.setRestUrl("https://testops.katalon.io/api/v1/test-projects/${nonExistingId}")
nonExistingRequest.setRestRequestMethod("GET")
addAuthHeader(nonExistingRequest)
def nonExistingResponse = WSBuiltInKeywords.sendRequest(nonExistingRequest)
WSBuiltInKeywords.verifyResponseStatusCode(nonExistingResponse, 404)
