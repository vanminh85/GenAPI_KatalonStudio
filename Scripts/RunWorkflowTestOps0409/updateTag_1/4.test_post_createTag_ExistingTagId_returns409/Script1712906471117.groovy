...
request2.setBodyContent(new HttpTextBodyContent(replaceSuffixWithUUID(JsonOutput.toJson(["name": "TestTeam", "role": "OWNER", "organizationId": organization_id]))))
...
request3.setBodyContent(new HttpTextBodyContent(replaceSuffixWithUUID(JsonOutput.toJson(["name": "TestProject", "teamId": team_id])))

def response3 = WSBuiltInKeywords.sendRequest(request3)
WSBuiltInKeywords.verifyResponseStatusCode(response3, 200)

def project_id = new JsonSlurper().parseText(response3.getResponseText())["id"]
...
request4.setBodyContent(new HttpTextBodyContent(replaceSuffixWithUUID(JsonOutput.toJson(["name": "TestTag", "projectId": project_id, "organizationId": organization_id])))
...
WSBuiltInKeywords.verifyResponseStatusCode(response5, 409)
