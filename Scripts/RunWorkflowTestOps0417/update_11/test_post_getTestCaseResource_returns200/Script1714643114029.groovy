...
payload_team = '{"name": "Team__unique__", "role": "OWNER", "organizationId": ' + response_org.getResponseText() + '.id}'
...
payload_project = '{"name": "Project__unique__", "teamId": ' + response_team.getResponseText() + '.id, "timezone": "UTC", "status": "ACTIVE", "canAutoIntegrate": true, "sampleProject": false}'
...
payload_test_case = '{"name": "Test Case__unique__", "path": "/test-case", "previousStatus": "PASSED", "alias": "TC", "testModuleId": 1, "webUrl": "https://test-case.com", "description": "This is a test case", "project": ' + response_project.getResponseText() + '}'
...
// Step 5: Retrieve the created Test Case by ID
test_case_id = new JsonSlurper().parseText(response_test_case.getResponseText())['id']
request_get_test_case = new RequestObject()
request_get_test_case.setRestUrl("https://testops.katalon.io/api/v1/test-cases/" + test_case_id)
request_get_test_case.setRestRequestMethod("GET")
addAuthHeader(request_get_test_case)
response_get_test_case = WSBuiltInKeywords.sendRequest(request_get_test_case)
WSBuiltInKeywords.verifyResponseStatusCode(response_get_test_case, 200)
